...
func main() {
	...
	go splitter(dataCh, evenCh, oddCh)
	go sum(evenCh, evenResultCh)
	go sum(oddCh, oddResultCh)
...
func splitter(dataCh chan int, evenCh chan int, oddCh chan int) {
	defer func() {
		close(evenCh)
		close(oddCh)
	}()
...
func sum(ch chan int, resultCh chan int) {
	result := 0
	for no := range ch {
		result += no
	}
...
func merge(evenResultCh chan int, oddResultCh chan int, resultFile string, processWg *sync.WaitGroup) {
